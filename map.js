...
config.chapters.forEach((record, idx) => {
  ...
  if (idx === config.chapters.length - 1) {
    container.classList.add("last-chapter"); // Add a specific class for the last chapter
  }
...
// Ensure scrolling stops after Chapter 4
scroller.onStepEnter((response) => {
  const chapterIndex = config.chapters.findIndex(
    (chapter) => chapter.id === response.element.id
  );
  if (chapterIndex >= 4) {
    // Disable further scrolling by removing the listener once Chapter 4 is reached
    scroller.removeStep(response.element);
  }
});
